...
\begin{itemize}
    ...
    \item open a fake website to capture the victim's username and password,
    \item create a communication backdoor by initializing a reverse shell.
\end{itemize}

The first on the list is \textbf{updating system configuration}. The attacker can create a payload that would for example change the network settings like changing the IP address of DNS server to redirect the requests to the attacker's location, disable an anti-virus program, firewall, or built-in hardware such as a touchpad or keyboard.

Another possible attack is to simply download malware from the internet and run it on the victim's computer (or run it from the mass storage). An example can be keylogger software. \textbf{Keylogger} is a program that runs in the background and its task is to capture anything the user type with its keyboard. The captured keystrokes can be uploaded to the remote server controlled by the attacker. The attack can then analyze received data and extract user's login credentials. Keylogger can be either a software program or a physical device (for example KeyGrabber\footnote{\url{https://www.keelog.com/keygrabber-keylogger/}}).

\todo{https://www.bleepingcomputer.com/news/security/fbi-hackers-use-badusb-to-target-defense-firms-with-ransomware/}

The attack can also create a payload that would perform a \textbf{Denial of Service} attack or execute a \textbf{Blue screen of Death} or \textbf{kernel panic}.
