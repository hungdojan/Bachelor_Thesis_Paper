...
The Universal Serial Bus has been with use for more than 20 years. 
% Computers have become more and more complex in recent times. In the 1940s last century, the world was gifted with the first computer. Not very powerful by today's standards, it took too much space since it was as big a building and it took a bunch of specialized people to operate it. 
% Ever since computers started to become more complex and compact people 
% Most people are familiar with USB which stands for Universal Serial Bus. If you buy an electronic device these days chances are you will encounter it. It's loved and hated by everyone. What do people love about it? It's easy to use! Doesn't matter whether you try to connect a keyboard, flash disk, your phone, or Arduino board, it just works immediately. It's like some kind of magic. 
...
% USB As for the time of this thesis, there are around 14 different USB connectors out there, USB-C being the most recent one.
...
That led to the development of a new interface. In 1995 a group named \textbf{USB Implementers Forum} (also known as USB-IF) was formed by these seven companies: Compaq, DEC, IBM, Intel, Microsoft, NEC and Nortel. Their aim was to create an interface with these goals in their mind:
\begin{itemize}
...
\begin{table}[ht]
    ...
    \begin{tabular}{|c|c|c|c|} \hline
         \multicolumn{2}{|c|}{\textbf{USB Version}} & \multirow{2}{*}{\textbf{Release date}} & \multirow{2}{*}{\thead{\textbf{Maximum} \\ \textbf{transmission rate}}} \\ \cline{1-2}
         ...
         USB 3.1 Gen 1   & SuperSpeed USB     &  \multirow{2}{*}{July 2013}   &  5 Gbps           \\ \cline{1-2} \cline{4-4}
         USB 3.1 Gen 2   & SuperSpeed+ USB    &                               &  10 Gbps          \\ \hline
         USB 3.2 Gen 1   & SuperSpeed USB     &  \multirow{3}{*}{August 2017} &  5 Gbps           \\ \cline{1-2} \cline{4-4}
         USB 3.2 Gen 2   & SuperSpeed 10 Gbps &                               &  10 Gbps          \\ \cline{1-2} \cline{4-4}
...
First two bytes of all descriptors stores two important information: \emph{bLength} declares the size of the descriptor/buffer that host received and \emph{bDescriptorType} tells the host what type of information can it expect in the buffer.

\textbf{Device} descriptor contains basic information about the device itself such as the version of USB used, device class, manufacture information, number of configurations of the device and more. It's the first descriptor that is sent to the host. In total the host will receive a buffer of 18 bytes. After the host retrieves the first descriptor it sends a request for a \textbf{configuration}, \textbf{interface} and \textbf{endpoint} descriptors. They specifies device's features and abilities. 

When developing device with version of USB being 2.0 or higher some additional data needs to be set. Since this version of USB allowed developer to choose what transfer speed to use, he also is responsible to define the device information and configuration of the speed not in use. And for that \textbf{device\_qualifier} and \textbf{other\_speed\_configuration} were created. The former contains \emph{device} descriptor of a device whose speed is currently not in use and the same goes for the latter one with \emph{configuration} descriptor. 
...
\textbf{String} descriptor contains descriptive text. Other descriptors can address each text using indices. For example if \emph{iManufacture} is set to \verb|0x03| it tells the host that the name of the manufacturer is stored as the fourth string descriptor. 

Other descriptors \-- \textbf{interface\_power}, \textbf{OTG}, \textbf{debug} \-- are not relevant for this thesis and their meanings can be found in the official documentations.

Table \ref{} displays the structure of each descriptor. The tables were taken from the Chapter 4 of the book called \emph{USB Complete: Everything You Need to Develop Custom USB Peripherals}. Listing \ref{} shows an example of a device descriptor's buffer.

\begin{table}[ht]
    \centering
    \begin{tabular}{|c|l|c|p{0.39\linewidth}|} \hline
        ...
        \multirow{8}{*}{configuration} & bLength & 1 & Descriptor size in bytes \\ \cline{2-4}
          & bDescriptorType & 1 & The constant CONFIGURATION (0x02) \\ \cline{2-4}
          & wTotalLength    & 2 & Sum of size of configuration, interface and endpoint descriptors
...
// device descriptor
    ...
    \end{verbatim}
    \caption{Example of a device descriptor}
    \label{fig:descriptor_example}
\end{figure}
User can find more detailed usage of each descriptor's field in the official documentation of USB specification. The example Figure \ref{fig:descriptor_example} shows the structure of a device descriptor's buffer. 
% TODO: make a reference to a USB specification
...
\chapter{Implementation of Rubber Ducky and Mass Storage}
