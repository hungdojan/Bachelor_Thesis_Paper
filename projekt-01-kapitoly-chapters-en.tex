...
Third group contains actions serves as a debug tool for the user. The debug cursor always starts at the beginning of the series. A request with \verb|GET_DEBUG_CURSOR| (0x06) will retrieve a \emph{key sequence} which is currently under the cursor. To get the next key sequence the user needs to send a \verb|INC_DEBUG_CURSOR| (0x07) which moves the cursor to the next one (if there is available). There \verb|RESET_DEBUG_CURSOR| (0x08) request moves the cursor back to the beginning.
