...
Next, I had to configure the Raspberry Pi Pico board to behave like a keyboard and mass storage simultaneously. As previously mentioned in \autoref{usb}, the host identifies the USB device's identify by retreiving its descriptor and configurations. TinyUSB defines 3 callback functions for that:
\begin{enumerate}
...
\end{itemize}
Unfortunately, due to Raspberry Pi Pico's lack of on board nonvolatile memory, I had to write a static file system from scratch. I chose a FAT12\todo{reference} file system since it is one of the simplest file system out there and for the proof of concept a sufficient one.

\textbf{FAT12} \todo{finish}.
